...
- Een ander getal dan 0, 90, 180 of 270 (bijv. `schiet(45)` of `schiet(360)`)
  is geen geldig commando. Hint:
  *"Schiet in 0, 90, 180 of 270 graden. 0 is vooruit, 90 omhoog, 180
  achteruit, 270 omlaag."*
- `robot = schiet(` en `robot = schiet(9` zijn nog onaf (Incomplete), geen
  fout.
- De bestaande hint bij een onbekend robot-commando ("Ik ken … niet. Probeer
  vooruit, achteruit, omhoog, omlaag of schiet.") verandert niet.
...
- **Parser:** alle vier hoeken geven `Shoot(graden)`; `robot = schiet` geeft
  ...
  Invalid met de graden-hint; `schiet(` en `schiet(9` zijn Incomplete.
- **Game:** `schiet(90)` raakt een robot recht boven de schutter binnen bereik;
  ...
  schutter staat, voor speler 2 gespiegeld; verticaal schieten aan de rand van
  het veld is "mis" met alleen de vakjes binnen het veld in `cellen`; Robo's
  `Shoot()` gedraagt zich als voorheen.
...
- `app/parser.py` – `Shoot.graden`, sjabloon `robot=schiet(#)`, `#` tot 3
  cijfers, hint voor foute graden.
